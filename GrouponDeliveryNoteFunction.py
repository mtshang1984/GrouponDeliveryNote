...
# 增加一个word表格列出商品派送名细
def add_building_order_table(building_order_data, this_document, groupon_owner, product_name, excel_column_name, building_number, max_building_number, number_line_in_page, max_row_number_per_page, if_hide_phone_number=True,if_upstream_park=False,if_use_pyqt=False,qtwidgets=None):
        ...
        if(product_building_order_data.shape[0] > 0):
            ...
            for index, row in product_building_order_data.iterrows():
                ...
                if(  row[excel_column_name["building_number"]]==555555):
                    ...
                    messagebox_text= "未能识别"+ row[excel_column_name["custom_name"]]+str(row[excel_column_name["phone_number"]])+"订单的楼号，请检查后再试！"
                    if if_use_pyqt:
                ...
                if(  row[excel_column_name["room_number"]]==555555):
                    ...
                    messagebox_text= "未能识别"+ row[excel_column_name["custom_name"]]+str(row[excel_column_name["phone_number"]])+"订单的房号，请检查后再试！"
                    if if_use_pyqt:
                        QMessageBox.critical(qtwidgets,"错误对话框",messagebox_text)
                    else:
                        print(messagebox_text)
...
def output_deliverynote_file(data, delivery_note_file_name, groupon_owner, product_name, excel_column_name, max_row_number_per_page, page_margin_cm, if_hide_phone_number=True,if_upstream_park=False,if_use_pyqt=False,qtwidgets=None):
    # 设置字体和段前段后行距
    ...
    #     if i not in 
        ...
        if building_order_data.shape[0] > 0:
            ...
            number_line_in_page = add_building_order_table(building_order_data, this_document, groupon_owner, product_name,
                                                           excel_column_name, i, max_building_number, number_line_in_page, max_row_number_per_page, if_hide_phone_number,if_upstream_park,if_use_pyqt,qtwidgets)
    ...
    try:
        this_document.save(delivery_note_file_name)
    except Exception as e:
        messagebox_text= "请关闭文件"+delivery_note_file_name+"后重试！"
        if if_use_pyqt:
            QMessageBox.critical(qtwidgets,"错误对话框",messagebox_text)
        else:
            print(messagebox_text)
...
def get_building_number_room_number_from_detial_adrees(detail_address,if_upstream_park=False,if_use_pyqt=False,qtwidgets=None):
    ...
    if len(detail_address)>2:#如果对楼号和室号进行分割后，获得了三个字符串，则只取最后两组字符串作为楼号和室号
        ...
        messagebox_text= "无法从'"+detail_address+"'解析出楼号和室号，请检查格式"
        if if_use_pyqt:
            QMessageBox.critical(qtwidgets,"错误对话框",messagebox_text)
        else:
            print(messagebox_text)
...
def main_program(input_file_name,if_use_pyqt=False,qtwidgets=None):
    ...
    if os.path.exists(input_file_name):
        ...
        messagebox_text= "未找到输入文件："+input_file_name
        if if_use_pyqt:
    ...
    if "order_file_name" in program_input:
        ...
        messagebox_text= "未指定订单文件"+order_file_name+"，请确认！"
        if if_use_pyqt:
            QMessageBox.critical(qtwidgets,messagebox_text)
        else:
            print(messagebox_text)            
    ...
    if "deliverynote_file_name" in program_input:
        ...
        deliverynote_file_name = str(PurePosixPath(order_file_name).parent)+"/"+Path(order_file_name).stem+"派送单.docx"
    ...
    # 输出派送单不带手机号
    reply=output_deliverynote_file(data, deliverynote_file_name, groupon_owner, product_name, excel_column_name,
                             max_row_number_per_page, page_margin_cm, if_hide_phone_number,if_upstream_park,if_use_pyqt,qtwidgets)
    if reply==True:
