...
class MainDialogHandle(QtWidgets.QDialog):
    ...
    def __init__(self,parent=None):
        ...
        self.listViewSequenceItemmodel.setStringList(["商品-楼号-房号","楼号-商品-房号","楼号-房号-商品","每件商品一标签","每户一标签","每件商品一标签（标签机）"]) 
        ...
        self.ui.lineEditLabelWidth.setText(self.label_width)
        self.ui.lineEditLabelHeight.setText(self.label_height)
        ...
        if os.path.exists(self.input_file_name):
            ...
            if "label_width" in program_input:
                self.label_width=program_input["label_width"]

            # 标签高度
            if "label_height" in program_input:
                self.label_height=program_input["label_height"]
        ...
        self.update_ui_from_variable()

    def update_ui_from_variable(self):
        self.ui.lineEditGrouponOwner.setText(self.groupon_owner)
    ...
    def set_widget_status(self):
        if self.show_sequence==1 or self.show_sequence==2:
            self.ui.listViewTitleSequence.setEnabled(True)
        else:
            self.ui.listViewTitleSequence.setEnabled(False)

        if self.show_sequence==6:
            self.ui.lineEditLabelHeight.setEnabled(True)
            self.ui.lineEditLabelWidth.setEnabled(True)
        else:
            self.ui.lineEditLabelHeight.setEnabled(False)
            self.ui.lineEditLabelWidth.setEnabled(False)
    ...
    def set_show_sequence(self):        
        ...
        if self.if_automated_set_output_path:
            self.deliverynote_file_name=generate_deliverynote_file_name(self.order_file_name,self.if_hide_phone_number,self.show_sequence)
            self.ui.lineEditDeliveryNote.setText(self.deliverynote_file_name)
        if self.show_sequence==4 or self.show_sequence==5 or self.show_sequence==6:
    ...
    def update_variable_from_ui(self):
        ...
        self.label_width=self.ui.lineEditLabelWidth.text()
        self.label_height=self.ui.lineEditLabelHeight.text()

    def save_config(self):
        self.update_variable_from_ui()
